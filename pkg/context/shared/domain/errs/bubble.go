...
func NewBubble(where, what string, why Meta, who error) *Bubble {
	if where == "" {
		service.PanicOnError("NewBubble", "cannot create a bubble if where is not defined")
	}

	if what == "" {
		service.PanicOnError("NewBubble", "cannot create a bubble if what is not defined")
	}
